...
class infoV2AE(object):
	def __init__(
		...
		self.info_optimizer = torch.optim.Adam(itertools.chain(self.actor.parameters(), self.info_posterior.parameters()),
												lr=ilr)
	...
	def train(self, replay_buffer, batch_size=256, envname=''):
		...
		if self.total_it %1000 ==0:
			print(self.total_it, "step: critic loss", critic_loss.item())
		...
		if self.total_it % self.policy_freq == 0:
			...
			with torch.no_grad():
				...
				latent, state_value = self.select_latent(state, self.actor_target)
				adv = Qmin - state_value
