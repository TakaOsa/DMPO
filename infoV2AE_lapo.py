...
class infoV2AE_lapo(object):
	def __init__(
		...
		self.info_posterior = PosteriorApproximator(state_dim, action_dim, latent_disc_dim,
													hidden=hidden,device_id=device_id).to(self.device)
		...
		self.info_optimizer = torch.optim.Adam(
			itertools.chain(self.actor.parameters(), self.info_posterior.parameters()), lr=ilr)
